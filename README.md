...
En n’oubliant pas de le mettre en globale de la classe pour pouvoir le récupérer autre part si besoin. 
```csharp
public double price;
```
Si la centrale rajoutée n’existe pas encore, il est très probable qu’il faille aussi rajouter/modifier la méthode Get_x_price().
```csharp
